...
declare const SqliteError: Database.SqliteErrorConstructor;
type SqliteError = Database.SqliteError;
type Statement<BindParameters extends unknown[] = Database.BindParameter[], Result = unknown> = Database.Statement<BindParameters, Result>;
type RunResult = Database.RunResult;
type Options = Database.Options;
type Transaction<F extends (...args: any[]) => any> = Database.Transaction<F>;
type ColumnDefinition = Database.ColumnDefinition;
type BackupMetadata = Database.BackupMetadata;
export {Database, SqliteError};
export type {Statement, RunResult, Options, Transaction, ColumnDefinition, BackupMetadata};
